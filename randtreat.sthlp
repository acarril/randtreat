...
{pstd}
{hi:randtreat} {hline 2} Randomly assign treatments uneven treatments and deal with misfits.

{title:Syntax}

{p 8 16 2}
{cmd:randtreat} [{varlist}]
[, {opt r:eplace} {opt so:rtpreserve} {opt se:tseed(#)} {opt mu:lt(integer)} {opt u:neven(fractions)} {opt mi:sfits(method)}]
{p_end}
...
It can handle an arbitrary number of treatments and uneven treatment fractions, which are common in real-world randomized control trials.
...
Also, uneven fractions of treatments may be specified using the {opt u:neven(fractions)} option.
...
{phang2}
{it: strata} randomly allocates misfits independently accross all strata, without weighting treatments as specified in {opt uneven}.
...
{phang2}
{it: global} randomly allocates all misfits globally, without weighting treatments as specified in {opt uneven}.
...
{phang2}
{it: wstrata} randomly allocates misfits independently accross all strata, weighting treatments as specified in {opt uneven}.
...
{phang2}
{it: wglobal} randomly allocates all misfits globally, weighting treatments as specified in {opt uneven}.
This ensures balance at the the global level and also respects uneven fractions of treatments, even when the number of misfits is large.
...
{title:Examples}
I suggest you {cmd:{help tabulate} {bf:treatment}, missing} after running each example.
...
Define stratification variables and uneven treatments, dealing with misfits:
...
Choose very unbalanced treatment fractions and dealing with misfits with and without weights:
	{cmd:randtreat, r uneven(2/5 1/5 1/5 1/5) mi(global) se(12345)}
	{cmd:randtreat, r uneven(2/5 1/5 1/5 1/5) mi(wglobal) se(12345)}
